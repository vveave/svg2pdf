...
"""
    ...
    for p in SVG_DIR.rglob("*"):
        if p.is_file() and p.suffix == ".svg":
            ...
            function_name = (
                re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", str(test_file.test_name()))
                .replace("/", "_")
                .replace("-", "_")
                .replace("=", "_")
                .replace(".", "_")
            ...
            if function_name in used_function_names:
                digest = hashlib.sha1(str(test_file.test_name()).encode()).hexdigest()[:8]
                function_name = f"{function_name}_{digest}"
